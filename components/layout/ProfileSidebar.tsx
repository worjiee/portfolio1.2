...
import { SidebarCertification } from "@/components/certifications/SidebarCertification";
...
export function ProfileSidebar() {
  return <aside aria-label="Profile" className="relative flex min-h-svh w-full items-center border-b border-[var(--border)] bg-[var(--sidebar)] px-5 py-16 md:fixed md:inset-y-0 md:left-0 md:w-[36%] md:overflow-hidden md:border-r md:border-b-0 md:px-8 md:py-6 lg:w-[34%]">
    <div className="profile-content mx-auto flex w-full max-w-[420px] flex-col md:my-auto">
      <Image src="/images/karl-pic.png" alt="Portrait of Karl" width={150} height={150} priority className="profile-image mb-10 size-[120px] self-center rounded-full object-cover object-top ring-[3px] ring-[var(--surface)] outline outline-1 outline-[var(--border)] shadow-[0_14px_32px_rgba(17,26,43,.12)] md:size-[142px]" />
      ...
      <SidebarCertification certification={certifications[0]} />
      <nav aria-label="Portfolio navigation" className="profile-nav my-7 flex flex-wrap gap-x-6 gap-y-3 border-y border-[var(--border)] py-5 font-[family-name:var(--font-mono)] text-[0.7rem] font-medium tracking-[.08em] text-[var(--muted)] uppercase">
        <Link className="transition-colors hover:text-[var(--accent)]" href="#work">Work</Link><Link className="transition-colors hover:text-[var(--accent)]" href="#certifications">Certifications</Link><Link className="transition-colors hover:text-[var(--accent)]" href="#about">About</Link><Link className="transition-colors hover:text-[var(--accent)]" href="#contact">Contact</Link>
      </nav>
