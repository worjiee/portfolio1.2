...
export function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return <motion.div initial={reduceMotion ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: reduceMotion ? 0 : .55, delay: index * .06 }}>
    <Link href={`/projects/${project.slug}`} className="group block min-w-0 text-inherit no-underline" aria-label={`View ${project.title} project`}>
      ...
      <div className="mt-5 mb-2 flex items-start justify-between gap-4"><h3 className="text-lg font-semibold">{project.title}</h3><ArrowUpRight className="text-[#718096] transition duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-[var(--accent)]" size={21} /></div>
      <p className="mb-4 text-sm leading-7 text-[var(--muted)]">{project.description}</p>
      <p className="mb-3 font-[family-name:var(--font-mono)] text-[0.6rem] font-medium tracking-[.06em] text-[#929cad] uppercase">{project.year} / {project.category}</p>
