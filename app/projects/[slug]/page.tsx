  ...
  return <>
    ...
    <main className="mx-auto max-w-[1320px] px-5 py-16 md:px-[5vw] md:py-[8vw]">
      <section className="mb-16 grid grid-cols-1 items-end gap-12 lg:grid-cols-[1.5fr_1fr] lg:gap-16">
        ...
        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-3">{[["Year", project.year], ["Role", project.role], ["Status", project.status]].map(([term, value]) => <div key={term} className="border-t border-[var(--border)] pt-3"><dt className="mb-2 font-[family-name:var(--font-mono)] text-[0.62rem] font-medium text-[var(--subtle)] uppercase">{term}</dt><dd className="m-0 text-[0.78rem] leading-6">{value}</dd></div>)}</dl>
      </section>
      ...
      <section className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        {project.githubUrl && <a href={project.githubUrl} target="_blank" rel="noreferrer" className="flex min-h-28 items-center justify-between rounded-2xl bg-[var(--button)] p-7 font-[family-name:var(--font-mono)] text-[var(--button-foreground)]">View GitHub <ArrowUpRight size={18} /></a>}
        {project.liveUrl ? <a href={project.liveUrl} target="_blank" rel="noreferrer" className="flex min-h-28 items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-7 font-[family-name:var(--font-mono)]">Live demo <ArrowUpRight size={18} /></a> : <span className="flex min-h-28 items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-7 font-[family-name:var(--font-mono)]">Live demo <small className="text-[var(--muted)]">Coming soon</small></span>}
      </section>
    </main>
    <footer className="flex flex-col items-start justify-between gap-7 bg-[var(--button)] px-[5vw] py-16 text-[var(--button-foreground)] sm:flex-row sm:items-center"><p className="font-[family-name:var(--font-mono)] text-[clamp(1.5rem,3vw,2.7rem)] font-medium">Have a project in mind?</p><Link href="/#contact" className="border-b border-[var(--subtle)] pb-1 font-[family-name:var(--font-mono)] text-xs">Let’s talk ↗</Link></footer>
  </>;
}
